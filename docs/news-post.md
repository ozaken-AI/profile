...
取得は掲載日降順。トップは8件、一覧・詳細の生成は既定500件まで。CMSのキーが未設定ならお知らせが空のままビルドは通るが、キーがあるのに取得できなければビルドは失敗する。空のまま公開すると既存の記事URLが一斉に404になるため、意図的に止めている。公開後にお知らせが欠落していないことを別に確認する。
