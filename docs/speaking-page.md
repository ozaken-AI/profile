...
入口の4形式 → 講演3テーマ → 研修4種類と関連する匿名実績 → 組織変革と関連する匿名実績 → 顧問と公開役職 → 公開の登壇・支援先 → 依頼の流れ → 共通FAQ → 相談先、の順。対象者と持ち帰る視点は折りたたまない。比較用のページ内ナビを途中でも使えるようにする。補足の論点・進め方・構成例・公開先の長い一覧は、同じページの `details` で開く。JavaScriptに依存するタブ切り替えは使わない。
...
- 企業実績は研修3件、全社推進2件を説明の近くに置く。旧 `#engagements` は研修の実績を含むセクション先頭への別名として維持する。
