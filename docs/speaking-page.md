...
入口の4形式 → 基調講演3テーマ → 研修4種類と関連する匿名実績 → 組織変革と関連する匿名実績 → 顧問と公開役職 → 公開の登壇・支援先 → 依頼の流れ → 共通FAQ → 相談先、の順。対象者と持ち帰る視点は折りたたまない。比較用のページ内ナビを途中でも使えるようにする。補足の論点・進め方・構成例・公開先の長い一覧は、同じページの `details` で開く。JavaScriptに依存するタブ切り替えは使わない。
...
- 基調講演の各テーマには相談、企画概要の印刷、共有リンクのコピーがある。印刷は同じページの `data-print-talk` とprint CSSで対象テーマだけを出す。別のサービス詳細ページや、手動同期するPDFを増やさない。印刷終了時に状態を解除する。
