...
> 2026年8月の取り込みでは、これが原因でした。**選択肢が空のまま**だったので、
> 何を入れても弾かれていました。下の手順で選択肢を足したら通りました。
...
**いちばん多い原因は、選択肢そのものが空になっていることです。**
スキーマをJSONでインポートすると、選択肢だけ入らないことがあります。
記事の編集画面でカテゴリを開いて「Select」しか出てこないなら、それです。
`docs/microcms-setup.md` の「`category` の選択肢」を見て足してください。

選択肢を足したあとは、まず `news-import-plain.csv`（値そのまま）から試してください。

選択肢が4つそろっているのに弾かれる場合は、**表記がこちらの想定と違います。**
...
### まとめて入れた直後は、ビルドの回数に注意

Webhook を設定した状態で232件を一括投入すると、**件数ぶんビルドが走ることがあります。**
Cloudflare Pages の無料枠は月500ビルドなので、Deployments を開いて
同じ内容のビルドが大量に積まれていないか見てください。

積まれている場合は、走り終わるのを待つか、いったん microCMS の Webhook を
オフにしてから最後に1回だけ手動でデプロイし直すのが早いです。
