...
| `thumbnailUrl` | `/news/<スラッグ>.webp`（画像はリポジトリに取り込み済み） |
...
**ただしこの場合、サムネイルは正しく出ます。** `thumbnailUrl` に画像の場所を直接書いてあるためです。
...
サムネイルの画像そのものを取り込み直す場合は `scripts/news-thumbs-fetch.mjs`、
そのあと `scripts/news-thumbs.mjs`（画像の寸法表を更新）を実行してください。
